...
def tuning(model_name, target_col, imputer):
    ...
    result_path = os.path.join("../results", target_col, model_name, f'impute_{imputer}_' + model_name + "_metrics.csv")
    best_param_path = os.path.join("../results", target_col, model_name, f'impute_{imputer}_' + model_name + "_best_params.json")
...
if __name__ == '__main__':
    ...
    with parallel_backend('threading', n_jobs=-1):
        tuning(args.model_name, args.target_col, args.imputer)
    end_time = datetime.now()
    print(f"Grid search test for {args.model_name}_{args.imputer} completes in {end_time - start_time}")
